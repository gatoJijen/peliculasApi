...
const data = await solicitud(`https://api.themoviedb.org/3/discover/movie?api_key=&with_genres=${documental}&page=1`)
console.log(data)
var  pag = 1
...
next.addEventListener('click', async () => {
    pag++;
    let data = await solicitud(`https://api.themoviedb.org/3/discover/movie?api_key=&with_genres=${documental}&page=${pag}`)
...
data.results.forEach(pelicula => {
    ...
    document.querySelector(".documental").addEventListener(".click", search(documental,solicitud,pag,paginacionA1,imprimirF))
});
