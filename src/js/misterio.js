...
const data = await solicitud(`https://api.themoviedb.org/3/discover/movie?api_key=&with_genres=${misterio}&page=1`)
console.log(data)
var  pag = 1
...
next.addEventListener('click', async () => {
    pag++;
    let data = await solicitud(`https://api.themoviedb.org/3/discover/movie?api_key=&with_genres=${misterio}&page=${pag}`)
...
data.results.forEach(pelicula => {
    ...
    document.querySelector(".misterio").addEventListener(".click", search(misterio,solicitud,pag,paginacionA1,imprimirF))
});
