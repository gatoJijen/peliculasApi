...
const data = await solicitud(`https://api.themoviedb.org/3/discover/movie?api_key=&with_genres=${CienciaFiccion}&page=1`)
console.log(data)
var  pag = 1
...
next.addEventListener('click', async () => {
    pag++;
    let data = await solicitud(`https://api.themoviedb.org/3/discover/movie?api_key=&with_genres=${CienciaFiccion}&page=${pag}`)
...
data.results.forEach(pelicula => {
    ...
    document.querySelector(".cienciaF").addEventListener(".click", search(CienciaFiccion,solicitud,pag,paginacionA1,imprimirF))
});
