...
const data = await solicitud(`https://api.themoviedb.org/3/discover/movie?api_key=&with_genres=${accion}&page=1`)
console.log(data)
var  pag = 1
...
next.addEventListener('click', async () => {
    pag++;
    let data = await solicitud(`https://api.themoviedb.org/3/discover/movie?api_key=&with_genres=${accion}&page=${pag}`)
...
data.results.forEach(pelicula => {
    ...
    document.querySelector(".accion").addEventListener(".click", search(accion,solicitud,pag,paginacionA1,imprimirF))
});
